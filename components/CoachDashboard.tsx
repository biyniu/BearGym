    ...
    <div className="min-h-screen bg-[#0a0a0a] flex text-gray-300 font-sans overflow-hidden">
            ...
            {filteredClients.map(c => (
              <div key={c.code} className="group relative flex items-center">
                <button onClick={() => loadClientDetail(c.code)} className={`flex-grow text-left p-3 rounded-xl transition mb-1 border ${selectedClient?.code === c.code ? 'bg-blue-600/10 border-blue-500 text-white' : 'bg-transparent border-transparent hover:bg-gray-800/50 text-gray-400'} ${c.status === 'inactive' ? 'opacity-40' : ''}`}>
                   <div className="font-bold text-[10px] uppercase italic flex items-center justify-between">
                     <span>{c.name}</span>
      ...
      <main className="flex-grow p-8 overflow-y-auto">
        {selectedClient ? (
          <div className="max-w-6xl mx-auto animate-fade-in">
            <header className="mb-10 flex flex-col md:flex-row justify-between items-start gap-6 border-b border-gray-800 pb-8">
              <div className="flex items-start space-x-6">
                <div className="w-16 h-16 bg-gray-800 rounded-2xl flex items-center justify-center text-3xl font-black text-white italic border border-gray-700 shadow-xl">{selectedClient.name.charAt(0)}</div>
                <div>
                  ...
                  <div className="flex items-center space-x-4">
                    <span className="bg-gray-800 px-2 py-0.5 rounded text-[10px] font-mono text-gray-400">ID: {selectedClient.code}</span>
                    <button onClick={toggleClientStatus} className={`text-[10px] font-black uppercase italic ${selectedClient.status === 'active' ? 'text-green-500' : 'text-red-500'}`}>
                      Status: {selectedClient.status === 'active' ? 'Aktywny' : 'Zarchiwizowany'}
                    </button>
                  </div>
                </div>
              </div>
              ...
              <div className="space-y-10">
                <div className="flex justify-end space-x-3 sticky top-0 z-10 bg-[#0a0a0a]/90 backdrop-blur py-4">
                   {!isEditingPlan ? (
                     ...
                     <>
                        <button onClick={handleSavePlan} className="bg-green-600 hover:bg-green-700 text-white px-8 py-4 rounded-2xl font-black uppercase italic text-xs shadow-xl transition active:scale-95">ZAPISZ</button>
                        <button onClick={() => setIsEditingPlan(false)} className="bg-gray-800 text-gray-500 px-8 py-4 rounded-2xl font-bold uppercase text-xs">ANULUJ</button>
                     </>
                   )}
                </div>
                    ...
                    <div className="space-y-2 overflow-x-auto">
                        ...
                        <div key={ex.id} className="grid grid-cols-12 gap-2 items-center bg-black/40 p-4 rounded-xl border border-gray-800 hover:border-gray-700 transition min-w-[800px]">
                           ...
                           <div className="col-span-1 flex justify-end">
                              {isEditingPlan && <button onClick={() => { workout.exercises.splice(idx,1); setEditedPlan({...editedPlan}); }} className="text-red-900 hover:text-red-500 transition"><i className="fas fa-trash text-xs"></i></button>}
                           </div>
                        </div>
                      ))}
            ...
            {activeTab === 'training' && (
                <div className="space-y-6 animate-fade-in">
                    {!activeTraining ? (
                        ...
                        <div className="bg-[#111] p-10 rounded-3xl border border-yellow-600/50 shadow-2xl">
                                ...
                                {selectedClient.plan[activeTraining.workoutId].exercises.map((ex: any) => (
                                    <div key={ex.id} className="bg-black/40 p-8 rounded-2xl border border-gray-800 hover:border-gray-700 transition group">
                                        <div className="flex justify-between items-center mb-8">
                                            <span className="text-2xl font-black text-white uppercase italic tracking-tight">{ex.name}</span>
                                            ...
                                            {Array.from({length: ex.sets}).map((_, sIdx) => (
                                                <div key={sIdx} className="space-y-3">
                                                    ...
                                                    <div className="flex space-x-2">
                                                      <input placeholder="kg" value={activeTraining.results[ex.id]?.[sIdx]?.kg || ''} onChange={(e) => updateLiveResult(ex.id, sIdx, 'kg', e.target.value)} className="w-1/2 bg-black border border-gray-800 text-white p-4 rounded-2xl text-center font-black text-sm focus:border-yellow-500 outline-none placeholder:text-gray-900 transition" />
                                                      <input placeholder="p" value={activeTraining.results[ex.id]?.[sIdx]?.reps || ''} onChange={(e) => updateLiveResult(ex.id, sIdx, 'reps', e.target.value)} className="w-1/2 bg-black border border-gray-800 text-white p-4 rounded-2xl text-center font-black text-sm focus:border-yellow-500 outline-none placeholder:text-gray-900 transition" />
                                                    </div>
                                                </div>
                                            ))}
                                        </div>
          ...
          <div className="h-full flex flex-col items-center justify-center opacity-5 text-center py-20"><i className="fas fa-bear-tracking text-[200px] mb-8"></i><h2 className="text-6xl font-black italic uppercase tracking-tighter">BEAR GYM SYSTEM</h2></div>
        )}
      </main>
